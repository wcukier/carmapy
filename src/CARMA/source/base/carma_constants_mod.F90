module carma_constants_mod
  ...
  implicit none
	...
	!! Define gravitational constant [ cm^3 / g / s ]
	real(kind=f), parameter :: GCONST = 6.67408e-8_f
	...
	!! Define proton mass [ g ]
	real(kind=f), parameter :: MPROT = 1.6726219e-24_f 
	...
	!! Define electronvolt in [ erg ]
	real(kind=f), parameter :: EV2ERG = 1.6e-12_f 

	!! Value of Collision Integral 
  	real(kind=f), parameter :: COLINT = 1._f   ! Collision integral, assume to be unity
  ...
  integer, parameter      :: CAM_FILL = -999
	...
	real(kind=f), parameter :: SMALL_PC = 1e-150_f
...
!	real(kind=f), parameter :: FEW_PC = tiny(ONE) * 1e6_f
	...
	real(kind=f), parameter :: FIX_COREF = 1e-50_f
